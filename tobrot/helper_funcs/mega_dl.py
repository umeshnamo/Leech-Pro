...
async def megadl(bot, message):
    ...
    try:
      if 'folder' in url:
        ...
        download_msg = await message.reply_text(f"**Trying To Download ...** \n\nPlease Wait Action Started 😜", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Cancel Mega DL", callback_data="cancel_mega")]]), reply_to_message_id=message.message_id)
    ...
    if lmaocheckdis > TG_MAX_FILE_SIZE:
        ...
        if not guessedfilemime.mime:
            ...
            await safone.reply_text(f"**©️ @DHKBots \nThanks For Using Me 🤖!**")
